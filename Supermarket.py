...
class Carrinho:
    ...
    def __str__(self):
        detalhes = "\n".join([f"{produto.nome} - {quantidade} unidade(s) - R${produto.preco * quantidade:.2f}" for produto, quantidade in self.itens])
        ...
        if opcao == 1:
            ...
            if produto:
                ...
                if quantidade <= produto.quantidade:
                    cliente = input("Digite o nome do cliente para adicionar ao carrinho: ")
                    cliente_obj = next((c for c in supermercado.clientes if c.nome == cliente), None)
                    if cliente_obj:
                        cliente_obj.adicionar_produto_ao_carrinho(produto, quantidade)
                        print(f"{quantidade} unidade(s) de {produto.nome} adicionada(s) ao carrinho de {cliente}.")
                    else:
